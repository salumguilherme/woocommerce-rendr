	...
	class Admin {
		...
		public function delivery_info_order_page($order) {
			...
			<p>Delivery ID: <?php echo get_post_meta($order->get_id(), 'rendr_delivery_id', true) ?><br>
				...
				<?php if(get_post_meta($order->get_id(), 'rendr_delivery_status', true) == 'requested') : ?><a href="https://retailer.rendr.delivery/<?php echo $this->get_method()->get_option('brand_id') ?>/deliveries/<?php echo get_post_meta($order->get_id(), 'rendr_delivery_id', true) ?>" target="_blank">Book Delivery</a>
		...
		public function rendr_t_and_c_check() {
			if(((is_array($_POST['shipping_method']) && strpos(serialize($_POST['shipping_method']), 'wcrendr') !== false) || (!is_array($_POST['shipping_method']) && strpos($_POST['shipping_method'], 'wcrendr') !== false)) && (empty($_POST['terms_wcrendr']))){
		...
		public function column_delivery_status_content($column) {
			...
			if($column == 'delivery_status') {
				...
				if(!empty(get_post_meta($post->ID, 'rendr_delivery_ref', true))) {
					echo '<br>Consignment No: '.get_post_meta($post->ID, 'rendr_delivery_ref', true).'';
				}
			}
		}
		...
		}//a href="https://retailer.rendr.delivery/'.$this->get_method()->get_option('brand_id').'/deliveries/'.get_post_meta($post->ID, 'rendr_delivery_id', true).'" target="_blank">View in Rendr portal</a>
		...
		public function order_actions($actions, $order) {
			...
			if($order->get_status() == 'processing' && !empty(get_post_meta($order->get_id(), 'rendr_delivery_id', true)) && (get_post_meta($order->get_id(), 'rendr_delivery_status', true) == 'requested')) {
				$actions['book'] = [
					'url' => 'https://retailer.rendr.delivery/'.$this->get_method()->get_option('brand_id').'/deliveries/'.get_post_meta($order->ID, 'rendr_delivery_id', true),
					'name' => 'Book',
					'action' => 'book',
				];
