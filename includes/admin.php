	...
	class Admin {
		...
		public function column_delivery_status_content($column) {
    		...
    		if($column == 'delivery_status') {
				...
				if(!empty(get_post_meta($post->ID, 'rendr_delivery_ref', true))) {
					echo '<br>Consignment No: '.get_post_meta($post->ID, 'rendr_delivery_ref', true).'<br><a href="https://retailer.rendr.delivery/plantsby_baker/deliveries/'.get_post_meta($post->ID, 'rendr_delivery_id', true).'" target="_blank">View in Rendr portal</a>';
				}
    		}
		}
		...
		public function request_delivery($order_id) {
			...
			foreach($methods as $method) {
				...
				if($method->get_method_id() !== 'wcrendr') {
					continue;
				}
				$rendr = new WC_Rendr_Delivery($method->get_instance_id());
				$rendr->request_delivery($order, $method->get_name());
			}
