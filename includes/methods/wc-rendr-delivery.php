...
/*				'label_flexible' => [
				...
				'disable_flexible' => [
					...
					'label' => 'Do not display "flex" delivery options to customers.',
...
/*				'label_standard' => [
			...
			$this->label_fast = 'Fast';
			$this->label_flexible = 'Flexible';
			$this->label_standard = 'Standard';
		...
		public function verify_credentials(array $args) {
			...
			if(wp_remote_retrieve_response_code($request) !== 200) {
				throw new \Exception(wp_remote_retrieve_response_message($request).' - Status code: '.wp_remote_retrieve_response_code($request));
			}
		 ...
		 * @throws \Exception
		 */
		private function get_ready_pickup_date() {
		...
		private function get_package_parcels($package) {
			...
			if($this->get_option('packing_preference') == 'together') {
				...
				foreach($package['contents'] as $lineitem) {
					$packer->addItem(new TestItem($lineitem['data']->get_name(), $this->get_item_shipping_attribute($lineitem, 'width'), $this->get_item_shipping_attribute($lineitem, 'length'), $this->get_item_shipping_attribute($lineitem, 'height'), $this->get_item_shipping_attribute($lineitem, 'weight'), false), $lineitem['quantity']);
				}
				...
				foreach ($packedBoxes as $index => $packedBox) {
					...
					$parcels[] = [
						'reference' => "Parcel #".str_pad($index+1, 4, '0', STR_PAD_LEFT),
						'description' => 'Type: '.$packedBox->getBox()->getReference().'. | Items: '.implode(', ', $packedItemsTitle),
						'length_cm' => $packedBox->getBox()->getOuterLength(),
						'height_cm' => $packedBox->getBox()->getOuterDepth(),
						'width_cm' => $packedBox->getBox()->getOuterWidth(),
						'weight_kg' => ($packedBox->getWeight()),
						'quantity' => 1,
					];
				}
		...
		private function can_calculate_shipping($package) {
			...
			if(empty($package['destination'])) {
				throw new \Exception('Cannot calculat postage. destination empty.');
			}

			if(empty($package['destination']['city'])) {
				throw new \Exception('Cannot calculat postage. city empty.');
			}

			if(empty($package['destination']['state'])) {
				throw new \Exception('Cannot calculat postage. state empty.');
			}

			if(empty($package['destination']['postcode'])) {
				throw new \Exception('Cannot calculat postage. postcode empty.');
			}
		...
		private function get_package_rates($package) {
			...
			$request = wp_remote_post($this->get_endpoint('/deliveries/quote'), [
				...
				'body' => json_encode([
					...
					'ready_for_pickup_at' => $this->get_ready_pickup_date(),
					'address' => [
			...
			foreach($data['data'] as $delivery_type => $rate) {
				...
				if($this->get_instance_option('disable_'.$delivery_type) > 0) {
					continue;
				}
		...
		public function request_delivery($order, $method_name) {
		...
		public function book_delivery($ref_id, $order) {
           ...
           $request = wp_remote_request($this->get_endpoint('/deliveries/'.$ref_id.'/book'), [
            ...
            //update_post_meta($order->get_id(), 'rendr_delivery_ref', 'RENDR000019');
